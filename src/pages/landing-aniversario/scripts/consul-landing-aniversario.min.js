...
Nitro.setup(['consul-landing-aniversario'], function () {
	...
	$('.header .container>div a:not(.share-facebook, .logo), #acoes-desktop .wrapper>div a:not(.share-facebook), #acoes-mobile .espiar').click(function(e){
		e.preventDefault();
		...
		validation.validate($fields, $btnSubmit)
							.done(function(){
								...
								$.getJSON(CRM.clientURI, {
									f: 'id,email,isNewsletterOptIn',
									fq: 'email:' + $inputEmail.val()
								}).done(function(res){
									if (res && res.isNewsletterOptIn) {
										//exibe msg de erro
										$('#cadastro .form .error').fadeIn();
									} else if (!res.isNewsletterOptIn) {
