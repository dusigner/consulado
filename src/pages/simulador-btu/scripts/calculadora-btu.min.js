...
jQuery( function () {
	...
	//quando inciar, scrolla a pagina pra calculadora ficar no topo da pagina e exibe botao 'fechar'
	jQuery( '.introducao .btn-init' ).on( 'click', function(){
		var top = jQuery(window).width() <= 768 ? (window.parent.jQuery('.page-calculadora-btu__banner').offset().top - window.parent.jQuery('header').height() + 60) : (window.parent.jQuery('.page-calculadora-btu__banner').offset().top);
		window.parent.jQuery('html, body').animate({ scrollTop: top }, 600);

		setTimeout(function(){
			window.parent.jQuery('#calculadora-btu').addClass('initiated');
			window.parent.jQuery('.page-calculadora-btu__banner').addClass('active');
		},1000);
	});
	...
	// no mobile, marca a primeira opcao dos input radios quando muda de slide
	jQuery( '.introducao .btn-init, .btn-seg' ).on( 'click', function(){
		if( ( config.dispositivo() == 'tablet' || config.dispositivo() == 'mobile' ) && ( jQuery( this ).hasClass( 'btn-init' ) ) ) {
			jQuery('.ativo fieldset label.radio:first-child input').click();
		}
	...
	jQuery( '.btn-no-input' ).on( 'click', function() {
		...
		jQuery( '.btn-seg.btn-required' ).removeClass( 'disabled' );

		//alterando o CSS pra deixar visivel a opcao escolhida
...
function Etapa(){
	...
	this.reiniciar = function(){
		...
		window.parent.$(window.parent.document).trigger('calculadora.reinit');
	};
