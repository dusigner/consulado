...
Nitro.module('upsell', function() {
	...
	let showNotAvaiable = $('td.value-field.Condicao-Indisponivel').text(),
	...
	this.init = () => {
		...
		$.ajax(setAPI)
			.then(function(responseDowngrade) {
	...
	this.renderUpsell = () => {
		...
		dust.render(template, apiResponse, function(err, out) {
			if (err) {
				throw new Error('upsell Dust error: ' + err);
			}
			$('#upsell').html(out);
		});
	...
	this.formatPrice = () => {
		...
		// prettier-ignore
		verifyDowngrade > 0
			? (priceBar = Number(apiResponseDowngrade.items[0].sellers[0].commertialOffer.Price))
			: (priceBar = Number(apiResponseUpgrade ? apiResponseUpgrade.items[0].sellers[0].commertialOffer.Price : '0'));
		priceBar < 1 && apiResponseUpgrade && apiResponseUpgrade.items[1]
			? (priceBar = Number(apiResponseUpgrade.items[1].sellers[0].commertialOffer.Price))
			: '';
		verifyDowngrade > 0 && priceBar < 1
			? (priceBar = Number(apiResponseDowngrade.items[1].sellers[0].commertialOffer.Price))
			: '';
		$('.title-price-upgrade span, .corpo-produtos-modal .oportunidadePro span, .price-mobile-upgrade').html(
			`R$ ${_.formatCurrency(priceBar)}`
		);
		...
		let // Pega, formata e subtrai os valores dos produtos na Barra fixa
			priceCurrent = $('.prod-preco .skuBestPrice')
				.text()
				.replace(/\D/gim, ''),
			priceUpgrade = $('.title-price-upgrade span')
				.text()
				.replace(/\D/gim, ''),
	...
	const isCheckedAndUnavailable = showNotAvaiable === 'Sim' && skuJson.available === false,
