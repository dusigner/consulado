...
Nitro.module('gallery-v3', function() {
	...
	this.init = () => {
		...
		if ( $('body').hasClass('testeAB-video-vitrine')) {
			...
			$('.galleryThumbs').parent().addClass('teste-video-B');
			$('.thumb-video').parent().addClass('video-b').find('img').remove();

		} else {
			$('.galleryThumbs').parent().removeClass('.teste-video-B');
			$('.galleryThumbs').parent().addClass('.teste-video-A');
		}
		...
		if(newThumbs.length > 8) {
			const lastSlide = $('.thumb').last();

			lastSlide.parent().html(`<li >
		 		<a class="thumb-ver-mais" onclick="$('.popup-zoom').magnificPopup('open', 6);">
