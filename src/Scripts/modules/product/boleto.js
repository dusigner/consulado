	...
	this.init = function() {
				...
				if (cmcDiscountCartao >= cmcDiscountBoleto) {
					...
					boletoInfo = `
						<p class="discount-boleto">
							À vista em <strong>${priceCash(sku.bestPrice, 'cartao')}</strong> <span>(${cmcDiscountCartao}% desconto)</span>
						</p>
					`;
				} else {
					...
					boletoInfo = `
						<p class="discount-boleto">
							À vista em <strong>${priceCash(sku.bestPrice, 'boleto')}</strong> <span>(${cmcDiscountBoleto}% desconto)</span>
						</p>
					`;
				}
		...
		if (prodAvailable.length > 0) {
			...
			if (cmcDiscountCartao >= cmcDiscountBoleto) {
				...
				boletoInfo = `
					<p class="discount-boleto">
						À vista em <strong>${priceCash(prodAvailable[0].bestPrice, 'cartao')}</strong> <span>(${cmcDiscountCartao}% desconto)</span>
					</p>
				`;
			} else {
				...
				boletoInfo = `
					<p class="discount-boleto">
						À vista em <strong>${priceCash(prodAvailable[0].bestPrice, 'boleto')}</strong> <span>(${cmcDiscountBoleto}% desconto)</span>
					</p>
				`;
			}
