				...
				$('.-buy-recurrence').on('click', function() {
					...
					vtexjs.checkout.getOrderForm().done(function(orderForm) {
						...
						if (recurrenceItemIndex > -1) {
							vtexjs.checkout
								.addItemAttachment(recurrenceItemIndex, 'Recorrência', {
									periodo: sku.period
								})
								.then(function() {
							...
							vtexjs.checkout
								.addToCart([recurrenceSku], null, window.jssalesChannel)
								.done(function(orderForm) {
									recurrenceItemIndex = orderForm.items.findIndex(
										e => e.id === sku.productSku.toString()
									);
									vtexjs.checkout
										.addItemAttachment(recurrenceItemIndex, 'Recorrência', {
											periodo: sku.period
