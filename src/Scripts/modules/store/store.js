...
define('store', function() {
	...
	this.init = function() {
		...
		this.isPersonal =
			/consul/.test(window.jsnomeLoja || window.vtex.accountName || window.location.host) &&
			!/consulempresa/.test(window.jsnomeLoja || window.vtex.accountName || window.location.host) &&
			!/consulqa2/.test(window.jsnomeLoja || window.vtex.accountName || window.location.host);

		this.isCorp =
			/consulempresa/.test(window.jsnomeLoja || window.vtex.accountName || window.location.host) ||
			/consulqa2/.test(window.jsnomeLoja || window.vtex.accountName || window.location.host);

		this.isQA = /consulqa/.test(
			window.jsnomeLoja ||
	...
	this.autoLogin = function() {
		if (this.userData.email && this.uri.path() === '/') {
			//pre-home
	...
	this.checkAccess = function() {
		this.isPrivateUrl = !publicUrl.some(
			function(item) {
