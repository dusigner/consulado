...
Nitro.module('busca-chaordic', function() {
	...
	this.pagination = function() {
		$('#list-more-products').on('click', function(e) {
			...
			self.getChaordicDataPagination($termPagination)
	...
	this.dropDown = function() {
		$('.search-multiple-navigator fieldset.even')
			.find('h5')
			.click(function() {
	...
	this.orderBy = function() {
		...
		$('ul.order-by-chaordic li a').on('click', function() {
			...
			self.getChaordicData($termFilter)
				.always(function(data) {
	...
	this.getVtexData = function($idProduto, el) {
		...
		var settings = {
			url:
				'//consul.vtexcommercestable.com.br/api/catalog_system/pub/products/search/?fq=productId:' + $idProduto,
			method: 'GET'
		};
		...
		return $.ajax(settings).done(function(response) {
			...
			function buildShelf() {
				...
				if (precoDe === 0 && response[0].items.length > 1) {
					...
					var objParcelas = self.prepareInstallments(
						response[0].items[1].sellers[0].commertialOffer.Installments
					);
					...
					objParcelas = self.prepareInstallments(
						response[0].items[0].sellers[0].commertialOffer.Installments
					);
				...
				el.find('.descricao-prod .price .por').html(
					'<span class="txtPor">Por: </span> <span class="val">R$ ' + _.formatCurrency(precoPor)
				);
				el.find('.descricao-prod .price .adicional').html(
					'<span class="vezes">' +
						numParcelas +
						'<span class="x">x</span></span> <span class="d">de</span> <span class="val">R$ ' +
						_.formatCurrency(valueParcela) +
						' </span><span class="total-parcelado"><span class="total-parcelado__txt hide">Total parcelado: </span><span class="val">' +
						_.formatCurrency(precoPor) +
						'</span></span>'
				);
				...
				if (promoPercentOff !== undefined) {
					...
					el.find('.descricao-prod .price .por').append(
						' <span class="off" style="display: inline;">' + promoPercentOff + '% OFF</span>'
					);
				}
				...
				$.each(clusterHighlights, function(index) {
					...
					el.find('.FlagsHightLight').append(
						'<p class="flag ' + clusterHighlights[index] + '">' + clusterHighlights[index] + '</p>'
					);
				});
			}
