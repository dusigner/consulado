...
Nitro.module('comparebar', function() {
	...
	this.triggerClick = function() {
		$(document).on('click', '.btn-comparar label', function() {
			var element = $(this)
					.parent()
					.find('.compare-product-checkbox'),
				skuProduto = $(this)
					.parent('.box-produto')
					.find('.nome .product-field ul li')
					.text();
			element.toggleClass('selected');
			dataLayer.push({
				event: 'generic',
				category: 'Categoria',
				action: 'Comparador de Produtos',
				label: skuProduto
			});
	...
	this.updateProduct = function() {
		...
		productsData.map(function(item, index) {
			template =
				'<div class="compare-bar__product-close js-remove-item" data-text="' +
				item.title +
				'" data-rel="' +
				item.rel +
				'"></div>' +
			...
			product.addClass('-has-product').html(template);
		});
	};
	...
	this.clearProducts = function() {
		...
		$('input.compare-product-checkbox')
