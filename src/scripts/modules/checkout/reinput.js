	...
	this.lockpurchase = function () {
		$('input#isReinput:checked').each(function () {
			...
			if (($('select#company').val() === 'Selecione') || ($('select#reason').val() === 'Selecione') || ($('select#alcada').val() === 'Selecione')) {
				setTimeout(function(){
					(!$('#payment-data-submit, #payment-data-submit:last-child').hasClass('disabled')) && $('#payment-data-submit, #payment-data-submit:last-child').attr('disabled', true);					
				}, 200);					
			} else{
				setTimeout(function(){						
					(!$('#payment-data-submit, #payment-data-submit:last-child').hasClass('disabled')) && $('#payment-data-submit, #payment-data-submit:last-child').attr('disabled', false);
				}, 200);
			}
		});
	};
